...
const ViewPatientPage = () => {
	...
	const [isAddConsultationModalOpen, setIsAddConsultationModalOpen] =
		useState(false);
	const [isEditConsultationModalOpen, setIsEditConsultationModalOpen] =
		useState(false);
	...
	const deleteConsultation = (pid, cid) => {
		...
		axiosPrivate
			.delete(`/patients/consultations/${cid}`)
			.then((response) => {
				console.log(response.data);
				setConsultations((consultations) =>
					consultations.filter((consultations) => consultations.id !== cid)
				);
	...
	useEffect(() => {
		...
		//get consultations
		axiosPrivate
			.get(`/patients/${pid}/consultations`, {
				signal: controller.signal,
			})
			.then((response) => {
				console.log(response.data);
		...
		<div>
			<div className="mb-2">
				...
				<button
					...
					className="mx-3 d-inline btn btn-outline-primary btn-sm"
				>
					<i className="bi bi-pencil-fill"></i>
				</button>
				<button
					...
					className="d-inline btn btn-outline-primary btn-sm"
				>
					<i className="bi bi-trash3-fill"></i>
				</button>
					...
					<div className="card-body text-dark">
						...
						<table style={{ tableLayout: "fixed", width: "100%" }}>
							<tbody>
								<tr>
									<td>
										<p className="card-text">Phone : {patientDetails?.phone}</p>
									</td>
									<td>
								...
								<tr>
									<td>
										<p className="card-text">DOB : {patientDetails?.dob}</p>
									</td>
									<td>
										<p className="card-text">City : {patientDetails?.city}</p>
									</td>
								</tr>
								<tr>
									<td>
									...
									<td>
										<p className="card-text">Notes : {patientDetails?.notes}</p>
									</td>
								</tr>
							</tbody>
						</table>
			...
			<CustomModal
				isModalOpen={isDeletePatientModalOpen}
				handleClose={() => setIsDeletePatientModalOpen(false)}
			>
				<p>Are you sure you want to delete this patient?</p>
				<button onClick={() => deletePatient(pid)} className="btn btn-danger">
					confirm
				</button>
			</CustomModal>
			<CustomModal
				isModalOpen={isDeleteConsultationModalOpen}
				handleClose={() => setIsDeleteConsultationModalOpen(false)}
			>
				<p>Are you sure you want to delete this consultation?</p>
				<button
					onClick={() => deleteConsultation(pid, selectedConsultation)}
			...
			<CustomModal
				isModalOpen={isEditPatientModalOpen}
				handleClose={() => setIsEditPatientModalOpen(false)}
			>
				<EditPatient
					patientDetails={patientDetails}
					patientId={pid}
					hideModal={() => setIsEditPatientModalOpen(false)}
					setPatientDetails={setPatientDetails}
				/>
			</CustomModal>
			<CustomModal
				isModalOpen={isAddConsultationModalOpen}
				handleClose={() => setIsAddConsultationModalOpen(false)}
			>
				<AddConsultation
					patientId={pid}
					hideModal={() => setIsAddConsultationModalOpen(false)}
					setConsultations={setConsultations}
				/>
			</CustomModal>
			<CustomModal
				isModalOpen={isEditConsultationModalOpen}
				handleClose={() => setIsEditConsultationModalOpen(false)}
			>
				<EditConsultation
					patientId={pid}
					consultationDetails={consultations.find(
						(c) => c.id === selectedConsultation
					)}
					hideModal={() => setIsEditConsultationModalOpen(false)}
					setConsultations={setConsultations}
